...
class ClassificationController extends Controller
{
    ...
    public function getConfusionMatrix()
    {
        try {
            ...
            $classes = $eval['classes'] ?? ['Negatif', 'Netral', 'Positif'];
            $cm_image = $eval['confusion_matrix_image'] ?? null;
    ...
    /**
     * Get wordcloud image as base64
     */
    public function getWordCloudImage()
    {
        try {
            $wordcloudPath = storage_path('app/private/wordcloud.b64');
            
            if (!file_exists($wordcloudPath)) {
                return response()->json([
                    'success' => false,
                    'message' => 'Wordcloud image not generated yet. Please train the model first.'
                ], 404);
            }
            
            $base64Image = file_get_contents($wordcloudPath);
            ...
            Log::error('Error getting wordcloud image', [
                'message' => $e->getMessage()
            ]);
            
            return response()->json([
                'success' => false,
                'message' => 'Error retrieving wordcloud image'
            ], 500);
        }
    }
    ...
    public function predictSentiment(Request $request)
    {
        try {
            ...
            if (!file_exists($scriptPath)) {
                throw new Exception('Prediction script tidak ditemukan');
            }

            // Escape text for command line
            $escapedText = str_replace('"', '\\"', $validated['text']);
            
            // Build command with proper escaping
            $pythonCmdEscaped = str_replace('/', '\\', $pythonCmd);
            $scriptPathEscaped = str_replace('/', '\\', $scriptPath);
            
            $cmd = "chcp 65001 > nul && \"{$pythonCmdEscaped}\" \"{$scriptPathEscaped}\" --text \"{$escapedText}\"";
            ...
            if ($stderr) {
                Log::debug('Python stderr output', ['stderr' => substr($stderr, 0, 500)]);
            }
            ...
            if (!isset($result['success']) || !$result['success']) {
                throw new Exception($result['error'] ?? 'Prediksi gagal');
            }

            Log::info('Sentiment prediction completed', [
            ...
            Log::error('Prediction error', ['message' => $e->getMessage()]);
            return response()->json([
                'success' => false,
                'message' => 'Error: ' . $e->getMessage()
            ], 500);
